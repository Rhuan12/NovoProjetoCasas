...
import { MaintenanceManagerLayout } from '@/components/MaintenanceManagerLayout'
...
function StatusBadge({ status }: { status: MaintenanceRequest['status'] }) {
  ...
  if (status === 'in_progress') return <Badge variant="default" size="sm">In Progress</Badge>
  return <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-success/20 text-success">Completed</span>
}
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="text-text-secondary mt-1">
              Manage requests from the resident portal
            </p>
          </div>
          <a
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          {[
            ...
            { label: 'In Progress', value: stats.inProgress, color: 'text-accent-primary', icon: Settings },
            { label: 'Completed', value: stats.completed, color: 'text-success', icon: CheckCircle },
            { label: 'Emergencies', value: stats.emergency, color: 'text-danger', icon: AlertTriangle },
          ].map(stat => {
        ...
        <Card className="p-6 space-y-4">
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1 relative">
              ...
              <input
                type="text"
                placeholder="Search by phone, address or description..."
                value={searchTerm}
              ...
              <div className="flex flex-wrap gap-2 items-center">
                ...
                {(['all', 'pending', 'in_progress', 'completed'] as StatusFilter[]).map(s => (
                  <Button key={s} variant={statusFilter === s ? 'primary' : 'outline'} size="sm" onClick={() => setStatusFilter(s)}>
                    {s === 'all' ? 'All' : s === 'pending' ? 'Pending' : s === 'in_progress' ? 'In Progress' : 'Completed'}
                  </Button>
                ))}
              </div>
              <div className="flex flex-wrap gap-2 items-center">
                <span className="text-xs text-text-muted w-20 flex-shrink-0">Category:</span>
        ...
        {loading ? (
          ...
          <Card className="p-12 text-center">
            ...
            <Button variant="outline" onClick={fetchRequests} className="mt-4">Try again</Button>
          </Card>
          ...
          <Card className="p-12 text-center">
            ...
            <p className="text-text-secondary">
              {searchTerm || statusFilter !== 'all' || categoryFilter !== 'all' || priorityFilter !== 'all'
                ...
                : 'Requests will appear here when submitted via the resident portal'}
            </p>
          </Card>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
            <Card className="w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden">
              <div className="flex-shrink-0 p-6 border-b border-background-tertiary flex items-center justify-between bg-background-secondary">
                <h2 className="text-xl font-bold text-text-primary">Request Details</h2>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-5">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <p className="text-xs text-text-muted mb-1">Preferred Visit Time</p>
